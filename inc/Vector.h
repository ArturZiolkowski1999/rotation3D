...
public:
    ...
    friend bool operator==(const Vector<T,dimension> &Vec1, const Vector<T,dimension> &Vec2);
    friend Vector<T,dimension> operator+(Vector<T,dimension> &Vec1, Vector<T,dimension> &Vec2);
    friend Vector<T,dimension> operator-(Vector<T,dimension> &Vec1, Vector<T,dimension> &Vec2);
    friend Vector<T,dimension> operator/(Vector<T,dimension> &Vec, const double &Num);
    friend Vector<T,dimension> operator*(Vector<T,dimension> &Vec, const double &Num);
    const T & operator[](int index) const;
    T & operator[](int index);
//
    T getLength();
    friend std::ostream & operator<<(std::ostream & ost, const Vector<T,dimension> &Vec);
...
T Vector<T, dimension>::getLength(){
    if(dimension < 3){
        double length = sqrt((this[0]) * (this[0]) + (this[1]) * (this[1]));
        return length;
    }
